...
from .model_utils import loader_2_dcrnn_fmt, dcrnn_fmt_2_loader
...
class EncoderModel(BaseModel, Seq2SeqAttrs):
    def __init__(self, adj_mx, **model_kwargs):
...
class DecoderModel(BaseModel, Seq2SeqAttrs):
    def __init__(self, adj_mx, **model_kwargs):
        BaseModel.__init__(self)
        Seq2SeqAttrs.__init__(self, adj_mx, **model_kwargs)
        self.output_dim = int(model_kwargs.get('output_dim'))
...
class DCRNNModel(BaseModel, Seq2SeqAttrs):
    def __init__(self, adj_mx, logger, add_storage, **model_kwargs):
        ...
        if self.add_storage:
            self.learn_stor = nn.Parameter(torch.ones(self.num_nodes) * 0.0001) 
        else:
    ...
    def forward(self, inputs, labels=None, batches_seen=None):
        ...
        if inputs.ndim == 4:  # (B, T, N, F)
            inputs = loader_2_dcrnn_fmt(inputs)
        ...
        outputs = dcrnn_fmt_2_loader(outputs, self.num_nodes, self.output_dim)
        if self.add_storage:
            return outputs, L_dir
        else:
            return outputs