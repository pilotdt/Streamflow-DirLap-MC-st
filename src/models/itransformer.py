...
class iTransformer(BaseModel):
    ...
    def __init__(self, enc_in, c_out, configs):
        ...
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            self.dec_embedding = DataEmbedding(self.c_out, self.d_model, configs.embed, configs.freq,
                                               configs.dropout)
        ...
        if self.task_name == 'classification':
            ...
            self.projection = nn.Linear(self.d_model * configs.seq_len, configs.num_class)
    ...
    def forecast(self, x_enc, x_mark_enc=None, x_mark_dec=None):
        ...
        # print(enc_out.shape)
        dec_out = self.dec_embedding(enc_out, x_mark_dec)
        # print(dec_out.shape)
        # dec_out = self.head(enc_out)  # z: [bs x nvars x target_window]
        dec_out = self.decoder(dec_out, enc_out, x_mask=None, cross_mask=None)
        return dec_out

    def imputation(self, x_enc, mask, x_mark_enc=None, x_mark_dec=None):
        # Embedding
        enc_out = self.enc_embedding(x_enc, x_mark_enc)
        enc_out, attns = self.encoder(enc_out, attn_mask=None)
    ...
    def forward(self, x_enc, x_mark_enc=None, x_mark_dec=None, mask=None):
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
        ...
        if self.task_name == 'imputation':
            dec_out = self.imputation(x_enc, mask, x_mark_enc, x_mark_dec)
            return dec_out  # [B, L, D]
        if self.task_name == 'anomaly_detection':
