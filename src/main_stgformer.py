"""
Example usage: python3 src/main_stgformer.py   num_runs=1
"""
...
def run_single_experiment(cfg, run_id, train_loader, val_loader, std_per_station, seq_lengths,
    ...
    # Initialize model
    supports = [A_norm] * cfg['history']  
    model = STGformer(
        num_nodes=num_stations,
        in_steps=cfg['history'],              
    ...
    # Evaluate model
    start_inf = time.time()
    preds_scaled, trues_scaled = evaluator.evaluate(test_loader)
    end_inf = time.time()
...
@hydra.main(version_base=None, config_path="../config", config_name="stgformer")
