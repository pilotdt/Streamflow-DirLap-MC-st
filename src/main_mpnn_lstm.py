"""
Example usage: python3 src/main_mpnn_lstm.py num_runs=1
"""
...
def run_single_experiment(cfg, run_id, train_loader, val_loader, std_per_station, seq_lengths,
    ...
    if cfg['reg_4_loss'] == "L_dir":   
        ...
        L_dir = None
        # edge_index, edge_weight = adjacency_to_edge_index(A_norm, row_ids, col_ids)  
    ...
    model = MPNN_LSTM(
        ...
        lambda_nl_reg=cfg["lambda_nl_reg"]).to(device)
...
@hydra.main(version_base=None, config_path="../config", config_name="mpnn_lstm")
def main(cfg: DictConfig):
    ...
    logger.info(f"Data prepared: {num_stations} stations/nodes, {in_features} features")
